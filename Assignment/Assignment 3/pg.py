...
def build_mlp(
  ...
  with tf.variable_scope(scope):
    prev_hidden = mlp_input
    for i in range(n_layers):
      hidden = tf.layers.dense(prev_hidden, size, activation=tf.nn.relu)
      prev_hidden = hidden
    output = tf.layers.dense(hidden, output_size, activation=output_activation)
...
class PG(object):
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    Continuous case:
        ...
        action_means: the predicted means for each action dimension.
            HINT: use build_mlp, check self.config for layer_size and n_layers
    ...
    if self.discrete:
      ...
      action_means = build_mlp(self.observation_placeholder,
                                ...
                                self.config.layer_size,
                                self.config.activation)
      log_std = tf.get_variable(name='log_std', shape=(1, self.action_dim))
      self.sampled_action = tf.random_normal(shape=(1, ), mean=action_means, stddev=log_std)
  ...
  def add_baseline_op(self, scope = "baseline"):
    ...
    self.baseline = tf.squeeze(build_mlp(self.observation_placeholder,
                                         self.action_dim,
                                         scope,
                                         self.config.n_layers,
                                         self.config.layer_size,
