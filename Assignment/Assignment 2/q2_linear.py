...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder(dtype=tf.uint8, shape=(None, img_height, img_width, nchannels*config.state_history), name='state')
        self.a = tf.placeholder(dtype=tf.int32, shape=(None), name='action')
        self.r = tf.placeholder(dtype=tf.float32, shape=(None), name='reward')
        self.sp = tf.placeholder(dtype=tf.uint8, shape=(None, img_height, img_width, nchannels * config.state_history), name='next_state')
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        out = tf.layers.dense(flatten, num_actions, name=scope, reuse=reuse)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        q_vars = tf.get_collection(key=tf.GraphKeys.GLOBAL_VARIABLES, scope=q_scope)
        tar_q_vars = tf.get_collection(key=tf.GraphKeys.GLOBAL_VARIABLES, scope=target_q_scope)
        assign_op = []
        for i in range(len(tar_q_vars)):
            assign_op.append(tf.assign(tar_q_vars[i], q_vars[i]))
    ...
    def add_loss_op(self, q, target_q):
        ...
        mask = tf.cast(tf.equal(tf.zeros_like(self.done_mask), self.done_mask), dtype=tf.float32)
        q_samp = self.r + mask * self.config.gamma * tf.reduce_max(target_q, axis=1)
        q_batch = tf.reduce_sum(tf.one_hot(self.a, num_actions) * q, axis=1)
        self.loss = tf.reduce_mean(tf.squared_difference(q_samp, q_batch))
    ...
    def add_optimizer_op(self, scope):
        ...
        vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope)
        gradients, variables = zip(*optim.compute_gradients(self.loss, vars))
        if self.config.grad_clip:
            grads_clip = [tf.clip_by_norm(grad, self.config.clip_val) for grad in gradients]
        self.train_op = optim.apply_gradients(zip(grads_clip, variables))
        self.grad_norm = tf.global_norm(grads_clip)
